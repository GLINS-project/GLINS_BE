package GLINS_BE.GLINS.auth.jwt.filter;
...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
     ...
     */
    private String reIssueRefreshToken(Client client) {
        String reIssuedRefreshToken = jwtService.createRefreshToken();
        client.updateRefreshToken(reIssuedRefreshToken);
        clientRepository.saveAndFlush(client);
        // redis에 저장
        redisTemplate.opsForValue().set(client.getEmail(), reIssuedRefreshToken, refreshTokenExpirationPeriod, TimeUnit.MILLISECONDS);
                ...
                .ifPresent(accessToken -> { jwtService.extractEmail(accessToken)
                        .ifPresent(email -> {
                            clientRepository.findByEmail(email)
                                    .ifPresentOrElse(
                                            client -> {
                                                this.saveAuthentication(client);
                                            },
                                            () -> {
     ...
     */
    public void saveAuthentication(Client myClient) {
        String password = myClient.getPassword();
        ...
        UserDetails userDetailsUser = User.builder()
                .username(myClient.getEmail())
                .password(password)
                .roles(myClient.getRole().name())
                .build();
